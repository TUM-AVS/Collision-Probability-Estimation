...
# TODO test vectorized implementation
...
def point_collvol_mc_sampling(
    ...
    #     poly2_boundary is not None
    # ), "Either point_coords or poly2_boundary must be provided."

    # assert poly1_boundary.ndim in [2, 3, 4] and poly1_boundary.shape[-1] in [
    #     2,
    #     3,
    # ], f"poly1_boundary must be Nx2 or Nx3 array of polygon vertices, got {poly1_boundary.shape}."
    ...
    if with_orientation and rel_cov is not None and rel_cov.shape[-1] >= 3:
        ...
        #     collvol, _ = collision_polygon(obs1, obs2_rot)
        # else:
        # TODO Proof that rotating obs1 also is correct?
        theta_std = np.sqrt(rel_cov[2, 2])
        delta_theta = norm(scale=theta_std).rvs(size=num_samples, random_state=SEED)
        rot_theta = np.zeros((*delta_theta.shape, 3, 3))
        costheta = np.cos(delta_theta)
...
def point_collvol_analytic(
    ...
    #     if with_orientation:
    #         theta_mean = poly2_boundary[..., 0, -1]
    #         theta_std = np.sqrt(rel_cov[-1, -1]) if rel_cov is not None else 0.0
    #         # convert nodes from [-1, 1] to [theta_mean - THETA_NSTD*theta_std, theta_mean + THETA_NSTD*theta_std]

    #         rot_theta = np.zeros((*theta_nodes.shape, 3, 3))
    #         rot_theta[..., 0, 0] = np.cos(theta_nodes)
    #         rot_theta[..., 0, 1] = np.sin(theta_nodes)
    #         rot_theta[..., 1, 0] = -np.sin(theta_nodes)
    #         rot_theta[..., 1, 1] = np.cos(theta_nodes)
    #         rot_theta[..., 2, 2] = 1.0
    #         poly2 = poly2_boundary @ rot_theta
    #         poly1 = poly1_boundary[None].repeat(poly2.shape[0], axis=0)
    #         collvol, point_coords = collision_polygon(poly1, poly2)
    #     else:
    #         collvol, point_coords = collision_polygon(poly1_boundary, poly2_boundary)
    #         # collvol = collvol[None]
    # else:
    #     if with_orientation and poly1_cov is not None and poly1_cov.shape[-1] >= 3:
    #         theta_mean = poly1_boundary[..., 0, -1]
    #         theta_std = np.sqrt(poly1_cov[-1, -1])
    #         # Linear transformation: theta = (thetah - thetal)/2 * node + (thetah + thetal)/2
    #         theta_nodes = THETA_NSTD * NODES_THETA * theta_std + theta_mean
    #         theta_jacobian = THETA_NSTD * theta_std
    #         rot_theta = np.zeros((*theta_nodes.shape, 3, 3))
    #         rot_theta[..., 0, 0] = np.cos(theta_nodes)
    #         rot_theta[..., 0, 1] = np.sin(theta_nodes)
    ...
    #         collvol = poly1_boundary @ rot_theta
    #         collvol[..., -1] = theta_nodes[:, np.newaxis]
    #     else:
    #         collvol = poly1_boundary[None]
